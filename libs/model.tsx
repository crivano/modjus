...
import { decodeHtmlEntities, handleSave, handleSaveWithValidations } from '@/libs/extension'
...
export function AlternativeModel(interview: (Frm: FormHelper) => JSX.Element, document: (data: any) => JSX.Element, options?: { saveButton?: boolean, pdfButton?: boolean, pdfFileName: string }) {
    const searchParams = useSearchParams()
    console.log('searchParams', JSON.stringify(searchParams))
    const currentUrl = searchParams.get('url') ? searchParams.get('url') as string : (process.env.NEXT_PUBLIC_URL || process.env.NEXT_PUBLIC_VERCEL_URL) + usePathname()
    // const initialData = searchParams.get('data') ? JSON.parse(searchParams.get('data') as string) : {}
        ...
        <div className="container-fluid">
                ...
                <div className="row">
                    <div id="modjus-interview" className="col col-12 col-md-6">
                        ...
                        <div className="d-flex justify-content-end mt-2">
                            {(!options || options.saveButton !== false) && <button className="btn btn-primary" onClick={handleSaveWithValidations}>Transportar</button>}
                        </div>
                    </div>
                    <div className="col col-12 col-md-6">
                        <h1 className="mt-3">Previsão do Documento</h1>
