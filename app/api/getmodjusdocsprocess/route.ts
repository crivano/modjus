...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const  num_processo_ou_documento = req.nextUrl.searchParams.get("num_processo");
    ...
    if (! num_processo_ou_documento || !nome_documento) {
    ...
    const conditions = nome_documento === process.env.FORM_UPDATE_OF_DAILY_RATES 
      ? `AND UPPER(s.nome) = UPPER("${nome_documento}") ORDER BY a2.dth_abertura DESC LIMIT 1` 
      : "AND p.protocolo_formatado = ? AND UPPER(s.nome) = UPPER(?)";
    ...
    const [rows] = await new Promise<[DocumentoConteudo[]]>(
      (resolve, reject) => {
        connection.query(
          `SELECT 
            a2.dth_abertura as data_assinatura, 
            v.versao as versao, 
            d.id_documento, 
          ...
          [ num_processo_ou_documento, nome_documento],
          (err, results) => {
            if (err) reject(err);
